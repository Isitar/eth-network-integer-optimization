...
\title{Network \& Integer Optimization – Problem set 2.2}
...
\begin{document}
	\section{Problem 2.2: A linear-time 2-approximation for vertex cover}
		
		\begin{definition}
			Let $\alpha \geq 1$. An $\alpha$-approximation algorithm for the vertex cover problem is an efficient algorithm that returns, for any input graph $G=(V,E)$, a vertex cover $S\subseteq V$ such that ${|S|\leq \alpha\cdot\tau(G)}$.
		\end{definition}
